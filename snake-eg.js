...
window.onload = function() {
  ...
  const TILE_INFO = { fill: 'grey', stroke:'black' , width: 2 }
  const food = { x: 0, y: 0, fill: 'red', stroke: 'black', flashingFactor: 0.1 };
 ...
 //           matrix[i][j-1]=1;
  ...
  const renderSnake = function(_snake) {
    for (let i = 0; i < _snake.parts.length; i ++) {
      ...
      rect(p.x, p.y, _snake.size, _snake.fill, _snake.stroke, 2);
    }
  };

  const renderFood = function() {
    food.flashingFactor += 0.1;
    rect(food.x, food.y, CELL_SIZE, food.fill, food.stroke, 2, Math.cos(food.flashingFactor));
  };
